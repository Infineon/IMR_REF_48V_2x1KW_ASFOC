...
volatile uint32_t Task10msCnt = 0;	// to update the 6EDL internal state and its registers
volatile uint32_t Task100msCnt = 0;	// to trigger CAN data transmission 
...
void MCU_RunISR0()
{
	...
	//----------------------------------------------------------------------------------
    //--- Position sensor readout acquisition
	//----------------------------------------------------------------------------------
    #ifdef USING_TLI_5012B
    Get_Pos_TLI_5012B(&TLI_5012B_ABS_POS, 0, motor[0].params_ptr->motor.P);

	#if OFFSET_CAL_DONE
    motor[0].vars_ptr->w_fb.elec    = TLI_5012B_ABS_POS.PLL.Omega_flt;
    motor[0].vars_ptr->th_r_fb.elec = TLI_5012B_ABS_POS.PLL.Theta_flt;
	#endif
    #endif
	//----------------------------------------------------------------------------------
...
void MCU_RunISR1()
{
	...
	Task10msCnt++;	// to update the 6EDL internal state and its registers
	Task100msCnt++;	// CAN timer to trigger transmission
...
RAMFUNC_BEGIN
static void DMA_ADC_1_RunISR() { // Task0
...
RAMFUNC_BEGIN
static void DMA_ADC_1_M1_RunISR() { // Task0
...
void MCU_RunISR0_M1()
{
	...
	//----------------------------------------------------------------------------------
	#ifdef USING_TLI_5012B
	Get_Pos_TLI_5012B(&TLI_5012B_ABS_POS_M1, 1, motor[1].params_ptr->motor.P);

	#if OFFSET_CAL_DONE
	motor[1].vars_ptr->w_fb.elec    = TLI_5012B_ABS_POS_M1.PLL.Omega_flt;
    motor[1].vars_ptr->th_r_fb.elec = TLI_5012B_ABS_POS_M1.PLL.Theta_flt;
	#endif
	#endif
	//----------------------------------------------------------------------------------
...
void MCU_RunISR1_M1()
{
    ...
    Cy_GPIO_Write(DIR_LED_M1_PORT, DIR_LED_M1_NUM, (motor[1].vars_ptr->dir == +1.0f));
#endif
    ...
    Cy_GPIO_Write(FAULT_LED_ALL_M1_PORT, FAULT_LED_ALL_M1_NUM, (bool)(motor[1].faults_ptr->flags_latched.all));
#endif
